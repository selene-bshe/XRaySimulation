...
import Projects.L10237.notebook_v2.MotorStack
...
class XppController_TG:
    ...
    def get_measurement(self, sase_field, kGrid, coor_dict):
        ...
        if self.cc_shutter:
            yag_image += np.reshape(DeviceSimu.get_intensity_on_YAG(intensity=simulation_statistics['cc sample intensity']['xy'],
                                                         intensity_coor=coor_dict,
                                                         intensity_loc=cc_trajectory_local[-1][:2],
                                                         pixel_coor={'xCoor':self.pixel_pos_x, 
                                                                     'yCoor':self.pixel_pos_y}),
                                    (self.pixel_num_x, self.pixel_num_y))
            print(cc_trajectory_local[-1][:2])
        if self.vcc_shutter:
            yag_image += np.reshape(DeviceSimu.get_intensity_on_YAG(intensity=simulation_statistics['vcc sample intensity']['xy'],
                                                         intensity_coor=coor_dict,
...
def get_optics(Ec=9.8):
    ...
    vcc_channel_cut_locations = np.zeros((4, 3))
...
def assemble_motors_and_optics(Ec=9.8):
    ...
    m1 = Projects.L10237_and_AutoAlign.notebook_v2.MotorStack.Mirror_tower1(mirror=optics_all['tg mirror probe'],
                                                                            crystal_loc=np.copy(optics_all['tg mirror probe'].surface_point),
                                                                            )
    m2a = Projects.L10237_and_AutoAlign.notebook_v2.MotorStack.Mirror_tower2(mirror=optics_all['tg mirror pump a'],
    ...
    sample = Projects.L10237_and_AutoAlign.notebook_v2.MotorStack.TG_Sample_tower(sample=optics_all['yag sample'],
